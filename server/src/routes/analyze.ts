...
import { runScoringEngine } from "../lib/scoringEngine";
...
router.get("/:ticker", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    let geminiData = {
      ...
      judgeSummary:
        "The AI committee synthesis is temporarily unavailable due to high demand. The recommendation and confidence scores above are fully calculated by our deterministic scoring engine using live Yahoo Finance data. Please refresh in a few minutes.",
    };
    ...
    const mock = mockTickerData[ticker];
    if (mock) {
